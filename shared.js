...
// ── Utilitaires ────────────────────────────────────────────
const NAV_DEFAULT_COLOR = (window.CURRENT_THEME&&window.CURRENT_THEME.neon)||'#00d4c8';
  ...
  return CE('div',{className:'listes-overlay',onClick:e=>{if(e.target.className==='listes-overlay')onClose();}},
    CE('div',{className:'listes-modal'},
      ...
      CE('div',{className:'listes-body'},
        activeTab==='conseillers'&&CE('div',{style:{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'8px 14px',marginBottom:8,background:'var(--bg-card2)',border:'1.5px solid var(--border)',borderRadius:8}},
          CE('span',{style:{fontSize:12,fontWeight:700,color:'var(--label-color)'}},'📧 Rappels email'),
          CE('button',{
            style:{fontSize:11,padding:'4px 10px',borderRadius:6,border:'1.5px solid #cbd5e0',background:'var(--card-bg)',cursor:'pointer',color:'var(--label-color)'},
...
function VueSaisie({entries,onSaved,onNewEntry,lists,editingId,onClearEdit,prefillData,onClearPrefill,accentColor}){
  ...
  return CE('div',{style:{padding:'4px 0'}},
    ...
    canToggle&&CE('div',{style:{display:'flex',gap:0,border:'1.5px solid var(--border)',borderRadius:8,overflow:'hidden',marginBottom:16,width:'fit-content'}},
    ...
    !modeLot&&CE('div',null,
      ...
      CE('div',{style:{display:'flex',gap:12,marginTop:4}},
        ...
        CE('button',{style:{padding:'13px 24px',border:'2px solid var(--border)',borderRadius:12,cursor:'pointer',fontSize:14,fontWeight:600,color:'var(--text-muted)',background:'var(--card-bg)'},onClick:reset},isDup||editId?'✖ Annuler':'✖ Réinitialiser')
      )
    ),
    ...
    modeLot&&CE('div',null,
      ...
      CE('div',{style:secStyle},
        ...
        CE('div',{style:{display:'grid',gridTemplateColumns:'140px 100px 64px 1fr 32px',gap:8,padding:'0 4px',marginBottom:6}},
          CE('span',{style:{fontSize:11,fontWeight:700,color:'var(--text-muted)'}},'DATE *'),
          CE('span',{style:{fontSize:11,fontWeight:700,color:'var(--text-muted)'}},'HORAIRE *'),
          CE('span',{style:{fontSize:11,fontWeight:700,color:'var(--text-muted)'}},'AM/PM'),
          CE('span',{style:{fontSize:11,fontWeight:700,color:'var(--text-muted)'}},'THÉMATIQUE *'),
          CE('span',null)
        ),
        lotRows.map((row)=>{
          ...
          return CE('div',{key:row.id,style:{display:'grid',gridTemplateColumns:'140px 100px 64px 1fr 32px',gap:8,alignItems:'start',padding:'9px 10px',borderRadius:10,border:`1.5px solid ${hasErr?'#fc8181':acLight}`,marginBottom:6,background:hasErr?'#fff5f5':acLight}},
            ...
            CE('select',{value:row.ampm,onChange:e=>setRow(row.id,'ampm',e.target.value),style:{width:'100%',padding:'8px 4px',border:brd(rErr.ampm),borderRadius:8,fontSize:12,background:'var(--bg-card2)'}},
              CE('option',{value:'',disabled:true},'—'),CE('option',{value:'AM'},'AM'),CE('option',{value:'PM'},'PM')),
        ...
        CE('button',{onClick:addRow,style:{display:'flex',alignItems:'center',justifyContent:'center',gap:6,padding:10,background:'var(--card-bg)',border:`2px dashed ${ac}`,borderRadius:10,cursor:'pointer',fontSize:13,color:ac,fontWeight:700,width:'100%',marginTop:6}},'＋ Ajouter une date')
      ),
      // Boutons
      CE('div',{style:{display:'flex',gap:12,marginTop:4}},
        ...
        CE('button',{style:{padding:'13px 24px',border:'2px solid var(--border)',borderRadius:12,cursor:'pointer',fontSize:14,fontWeight:600,color:'var(--text-muted)',background:'var(--card-bg)'},onClick:resetLot},'✖ Réinitialiser')
      )
    )
  );
}
...
function VueHistorique({entries,onEdit,onDelete,onRefresh,onDuplicate,initConseiller,onResetConseiller,canDelete,onChangeConseiller}){
  ...
  return CE('div',null,
    ...
    CE('div',{className:'kpi-row'},
      CE(FadeItem,{delay:0,style:{flex:'1 1 0',minWidth:0}},CE('div',{className:'kpi-mini',style:{borderLeft:'3px solid #1e3a8a',background:'#f0f4ff'}},CE('div',{className:'v',style:{color:'var(--neon)'}},kpi.total),CE('div',{className:'l'},'Total'))),
    ...
    CE('div',{className:'card',style:{marginBottom:10}},
      CE('div',{style:{fontSize:11,fontWeight:700,color:'var(--text-muted)',letterSpacing:'.06em',marginBottom:6}},'STATUT'),
      ...
      CE('div',{style:{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}},
        CE('input',{type:'text',value:search,placeholder:'🔍 Recherche…',style:{flex:'1 1 160px',padding:'6px 10px',border:'1.5px solid var(--border)',borderRadius:6,fontSize:13},onChange:e=>setSearch(e.target.value)}),
        CE('select',{style:{padding:'6px 8px',border:'1.5px solid var(--border)',borderRadius:6,fontSize:12},value:filtMois,onChange:e=>setFiltMois(e.target.value)},
          CE('option',{value:'Tous'},'Tous les mois'),moisDispo.map(m=>CE('option',{key:m,value:m},m))),
        CE('select',{style:{padding:'6px 8px',border:'1.5px solid var(--border)',borderRadius:6,fontSize:12},value:filtCommune,onChange:e=>setFiltCommune(e.target.value)},
          CE('option',{value:'Toutes'},'Toutes communes'),COMMUNES.map(c=>CE('option',{key:c,value:c},c))),
        CE('select',{style:{padding:'6px 8px',border:'1.5px solid var(--border)',borderRadius:6,fontSize:12},value:filtConseiller,onChange:e=>{setFiltConseiller(e.target.value);if(onChangeConseiller)onChangeConseiller(e.target.value);}},
      ...
      CE('div',{style:{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center',marginTop:8}},
        CE('div',{style:{display:'flex',alignItems:'center',gap:4}},
          CE('span',{style:{fontSize:11,fontWeight:700,color:'var(--text-muted)',whiteSpace:'nowrap'}},'Du'),
          CE('input',{type:'date',value:dateFrom,onChange:e=>setDateFrom(e.target.value),style:{padding:'6px 8px',border:'1.5px solid var(--border)',borderRadius:6,fontSize:12}}),
          CE('span',{style:{fontSize:11,fontWeight:700,color:'var(--text-muted)',whiteSpace:'nowrap'}},'Au'),
          CE('input',{type:'date',value:dateTo,onChange:e=>setDateTo(e.target.value),style:{padding:'6px 8px',border:'1.5px solid var(--border)',borderRadius:6,fontSize:12}})
        ),
      ...
      CE('div',{style:{fontSize:11,color:'var(--text-muted)',marginTop:8}},`${filtered.length} atelier(s) affiché(s) sur ${entries.length}`)
    ),
    // Liste des ateliers
    ...
    CE('div',{className:'side-panel'+(panel?' open':'')},
      panel&&CE(React.Fragment,null,
        CE('div',{className:'side-panel-header'},
          ...
          CE('button',{onClick:closePanel,style:{background:'none',border:'none',fontSize:18,cursor:'pointer',color:'var(--text-muted)'}},'✕')
        ),
        CE('div',{className:'side-panel-body'},
          ...
          panel.materiel&&panel.materiel.length>0&&CE('div',{style:{marginTop:10,marginBottom:4}},
            CE('div',{style:{fontSize:11,fontWeight:700,color:'var(--text-muted)',marginBottom:4}},'MATÉRIEL'),
    ...
    confirmDel&&CE('div',{style:{position:'fixed',inset:0,background:'rgba(0,0,0,.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000}},
      CE('div',{className:'card',style:{width:360}},
        ...
        CE('p',{style:{fontSize:12,color:'var(--text-muted)',marginBottom:16}},'Cette action est irréversible.'),
    ...
    CE('div',{className:'card',style:{marginBottom:12}},
      CE('div',{style:{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10,marginBottom:12}},
        CE('div',{style:{display:'flex',alignItems:'center',gap:8}},
          ...
          CE('div',{style:{fontSize:18,fontWeight:800,color:'var(--neon)',minWidth:170,textAlign:'center'}},`${MOIS_LONG[mo]} ${yr}`),
        ...
        CE('div',{style:{display:'flex',alignItems:'center',gap:8}},
          CE('label',{style:{fontSize:12,fontWeight:600,color:'var(--label-color)',margin:0,whiteSpace:'nowrap'}},'Conseiller :'),
          CE('select',{value:filtConseiller,onChange:e=>{setFiltConseiller(e.target.value);if(onChangeConseiller)onChangeConseiller(e.target.value);},style:{padding:'5px 10px',border:'1.5px solid var(--border)',borderRadius:6,fontSize:13}},
      ...
      CE('div',{style:{display:'flex',gap:8,flexWrap:'wrap'}},
        CE('div',{className:'kpi-mini',style:{flex:'1 1 70px',borderLeft:'3px solid #1e3a8a',background:'#f0f4ff'}},CE('div',{className:'v',style:{color:'var(--neon)',fontSize:20}},kpi.total),CE('div',{className:'l'},'Ateliers')),
...
// ═══════════════════════════════════════════════════════════
function NoData(){return CE('p',{style:{color:'var(--text-muted)',fontSize:12,textAlign:'center',paddingTop:20}},'Aucune donnée');}
...
const EC_AXIS_LABEL={color:'var(--text-muted)',fontSize:10};
...
function BarChart({data,colors,height}){
  ...
  const option={
    ...
    series:[{type:'bar',barMaxWidth:44,
      ...
      label:{show:true,position:'top',color:'var(--text-muted)',fontSize:10,fontWeight:'bold'}
    }]
  };
...
function LineChart({data}){
  ...
  const option={
    ...
    series:[{type:'line',data:data.map(d=>d.value),smooth:true,symbol:'circle',symbolSize:5,
      ...
      label:{show:true,position:'top',color:'var(--text-muted)',fontSize:9,fontWeight:'bold'}
    }]
  };
...
function DualLineChart({data}){
  ...
  const option={
    ...
    tooltip:{trigger:'axis',...EC_TT,...EC_APN},
    legend:{data:['Inscrits','Présents'],textStyle:{color:'var(--text-muted)',fontSize:11},bottom:0,icon:'circle',itemWidth:8,itemHeight:8},
    xAxis:{data:data.map(d=>d.label),...EC_AXIS_BASE,splitLine:{show:false},axisLabel:{...EC_AXIS_LABEL,rotate:data.length>8?-35:0,interval:0}},
    yAxis:{...EC_AXIS_BASE},
    series:[
...
function RadialChart({data,colors,height=220}){
  ...
  const option={
    ...
    series:[{type:'bar',barMaxWidth:32,
      ...
      label:{show:true,position:'right',color:'var(--text-muted)',fontSize:10,fontWeight:'bold'}
    }]
  };
...
function DonutChart({data,colors,height=220}){
  ...
  const option={
    ...
    legend:{orient:'horizontal',bottom:0,textStyle:{color:'var(--text-muted)',fontSize:11},icon:'circle',itemWidth:8,itemHeight:8},
...
function StackedActivityChart({data}){
  ...
  const option={
    ...
    legend:{data:['Réalisés','Annulés','Planifiés'],textStyle:{color:'var(--text-muted)',fontSize:10},bottom:0,icon:'roundRect',itemWidth:10,itemHeight:8},
...
function ConseillerBarChart({entries}){
  ...
  const option={
    ...
    legend:{data:['Réalisés','Planifiés','Annulés'],textStyle:{color:'var(--text-muted)',fontSize:10},bottom:0,icon:'roundRect',itemWidth:10,itemHeight:8},
    xAxis:{...EC_AXIS_BASE},
    ...
    series:[
      {name:'Réalisés',type:'bar',barMaxWidth:14,data:data.map(d=>d.realises),
        ...
        label:{show:true,position:'right',color:'var(--text-muted)',fontSize:9,fontWeight:'bold'}},
...
function KpiCard({val,lbl,sub,trend,color,icon,bgColor,delay=0}){
  ...
  return CE('div',{className:'kpi',style:{background:bgColor||'#fff',borderLeft:'4px solid '+(color||'#1e3a8a'),textAlign:'left',padding:'14px 16px',position:'relative',overflow:'hidden',opacity:sv?1:0,transform:sv?'none':'translateY(14px)',transition:'opacity .45s ease,transform .45s ease'}},
    ...
    CE('div',{style:{display:'flex',alignItems:'flex-start',justifyContent:'space-between'}},
      CE('div',null,
        CE('div',{style:{fontSize:11,color:'var(--text-muted)',fontWeight:600,marginBottom:4,display:'flex',alignItems:'center',gap:4}},icon,' ',lbl),
        CE('div',{className:'val',style:{color:color||'#1e3a8a',fontSize:26,lineHeight:1.1}},val),
        sub&&CE('div',{style:{fontSize:11,color:'var(--text-muted)',marginTop:3}},sub)
      ),
...
function VueDashboard({entries}){
  if(!window.echarts)return CE('div',{className:'card'},CE('p',{style:{color:'var(--text-muted)',textAlign:'center',padding:'40px 0'}},'⚠️ ECharts non chargé.'));
  ...
  const accent=getComputedStyle(document.documentElement).getPropertyValue('--neon').trim()||'#1e3a8a';
  ...
  return CE('div',null,
    ...
    CE('div',{className:'card',style:{marginBottom:12,padding:'12px 16px'}},
      CE('div',{style:{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}},
        CE('span',{style:{fontSize:12,fontWeight:700,color:'var(--text-muted)'}},'Période :'),
        ...
        CE('span',{style:{marginLeft:'auto',fontSize:11,color:'var(--text-muted)'}},filtered.length+' ateliers · '+now.toLocaleDateString('fr-FR'))
      )
    ),
...
function VueGraphiques({entries}){
  if(!window.echarts)return CE('div',{className:'card'},CE('p',{style:{color:'var(--text-muted)',textAlign:'center',padding:'40px 0'}},'⚠️ ECharts non chargé — vérifiez la connexion internet.'));
  ...
  return CE('div',null,
    ...
    CE('div',{className:'card',style:{marginBottom:12}},
      CE('div',{style:{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}},
        CE('span',{style:{fontSize:12,fontWeight:700,color:'var(--text-muted)',whiteSpace:'nowrap'}},'📅 Période'),
        // Boutons années
        ...
        CE('div',{style:{display:'flex',alignItems:'center',gap:6,marginLeft:4}},
          CE('span',{style:{fontSize:12,color:'var(--text-muted)'}},'Du'),
          CE('input',{type:'date',value:dateFrom,onChange:e=>setDateFrom(e.target.value),style:{padding:'5px 8px',border:'1.5px solid var(--border)',borderRadius:6,fontSize:12}}),
          CE('span',{style:{fontSize:12,color:'var(--text-muted)'}},'Au'),
          CE('input',{type:'date',value:dateTo,onChange:e=>setDateTo(e.target.value),style:{padding:'5px 8px',border:'1.5px solid var(--border)',borderRadius:6,fontSize:12}})
        ),
        hasFilter&&CE('span',{style:{fontSize:12,color:'var(--neon)',fontWeight:600,background:'#eff6ff',padding:'4px 10px',borderRadius:6}},`${filtered.length} / ${entries.length}`),
        CE('button',{className:'btn btn-print btn-sm',style:{marginLeft:'auto'},onClick:()=>window.print()},'🖨️ Imprimer')
      )
    ),
    passes.length===0
      ?CE('div',{className:'card'},CE('p',{style:{color:'var(--text-muted)',textAlign:'center',padding:'40px 0'}},hasFilter?'Aucun atelier réalisé sur cette période.':'Aucun atelier réalisé.'))
      :CE('div',null,
...
function VueCarte({entries,active}){
  ...
  return CE('div',null,
    ...
    CE('div',{className:'card'},
      ...
      CE('div',{style:{display:'flex',gap:16,marginBottom:10,flexWrap:'wrap',fontSize:12,color:'var(--label-color)'}},
...
function VueBingo({entries}){
  ...
  return CE('div',null,
    CE('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}},
      CE('div',{style:{fontSize:13,color:'var(--text-muted)'}},
    ...
    sel&&CE('div',{className:'card'},
      CE('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}},
        CE('h2',{style:{borderBottom:'none',marginBottom:0,paddingBottom:0}},CE('span',{style:{color:'var(--neon)'}},'📍 '+sel.nom),CE('span',{style:{fontSize:13,fontWeight:400,color:'var(--text-muted)',marginLeft:8}},'— '+sel.total+' atelier(s)')),
        CE('button',{onClick:()=>setSelected(null),style:{background:'none',border:'none',fontSize:18,cursor:'pointer',color:'var(--text-muted)'}},'✕')
      ),
  ...
  return CE('div',{ref:adminRef},
    CE('div',{className:'card'},
      ...
      CE('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginBottom:8}},
        CE(FadeItem,{delay:0,style:{display:'contents'}},CE('div',{className:'kpi',style:{borderLeft:'4px solid #1e3a8a',background:'#f0f4ff',textAlign:'left'}},CE('div',{className:'val',style:{color:'var(--neon)'}},entries.length),CE('div',{className:'lbl'},'Total ateliers'))),
    ...
    CE('div',{className:'admin-section'},
      ...
      CE('p',{style:{fontSize:12,color:'var(--label-color)',marginBottom:16}},'Personnalisez la couleur de chaque conseiller numérique. S\'applique au bandeau, au formulaire et aux cartes.'),
      CE('div',{style:{display:'flex',flexDirection:'column',gap:10}},
        (conseillersList||Object.keys(CONSEILLER_COLORS)).map(c=>{
          ...
          return CE('div',{key:c,style:{display:'flex',alignItems:'center',gap:12,padding:'10px 14px',background:'var(--bg-card2)',borderRadius:10,border:'1.5px solid var(--border)'}},
            // Color picker visuel
            ...
            CE('input',{type:'text',value:hexVal,maxLength:7,
              ...
              style:{width:88,fontFamily:'monospace',fontSize:13,fontWeight:700,color:'var(--text)',padding:'6px 10px',border:'1.5px solid var(--border)',borderRadius:8,background:'var(--card-bg)',textAlign:'center'}}),
            // Bouton reset
            CE('button',{onClick:()=>{const orig=CONSEILLER_COLORS[c]||'#6B7280';setColorDraft(d=>({...d,[c]:orig}));},
              style:{padding:'4px 10px',border:'1px solid var(--border)',borderRadius:6,fontSize:11,color:'var(--text-muted)',cursor:'pointer',background:'var(--card-bg)',flexShrink:0}},'↩ Reset')
          );
        })
      ),
    ...
    visibility&&CE('div',{className:'admin-section'},
      ...
      CE('p',{style:{fontSize:12,color:'var(--label-color)',marginBottom:12}},'Choisissez les onglets visibles dans l\'interface conseiller.'),
    ...
    CE('div',{className:'admin-section'},
      ...
      CE('p',{style:{fontSize:12,color:'var(--label-color)',marginBottom:12}},'Importe un fichier CSV compatible. Les entrées existantes sont fusionnées.'),
      importing?CE('div',null,CE('p',{style:{fontSize:12,color:'var(--label-color)',marginBottom:6}},importMsg),CE('div',{className:'progress-bar'},CE('div',{className:'progress-fill',style:{width:importProgress+'%'}})),CE('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:6}},CE('p',{style:{fontSize:11,color:'var(--text-muted)'}},importProgress+'%'),CE('button',{className:'btn btn-danger btn-sm',onClick:()=>cancelRef.current=true},'⛔ Annuler'))):CE('label',{style:{display:'inline-block',cursor:'pointer'}},CE('span',{className:'btn btn-primary'},'📂 Choisir un fichier CSV'),CE('input',{type:'file',accept:'.csv',style:{display:'none'},onChange:handleImportCSV}))
    ),
    CE('div',{className:'admin-section'},
      CE('h3',null,'📊 Import XLSX'),
      CE('p',{style:{fontSize:12,color:'var(--label-color)',marginBottom:12}},'Réimporte un fichier .xlsx précédemment exporté.'),
      importing?CE('div',null,CE('p',{style:{fontSize:12,color:'var(--label-color)',marginBottom:6}},importMsg||'Import en cours…'),CE('div',{className:'progress-bar'},CE('div',{className:'progress-fill',style:{width:importProgress+'%'}})),CE('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginTop:6}},CE('p',{style:{fontSize:11,color:'var(--text-muted)'}},importProgress+'%'),CE('button',{className:'btn btn-danger btn-sm',onClick:()=>cancelRef.current=true},'⛔ Annuler'))):CE('label',{style:{display:'inline-block',cursor:'pointer'}},CE('span',{className:'btn btn-warn'},'📂 Choisir un fichier XLSX'),CE('input',{type:'file',accept:'.xlsx',style:{display:'none'},onChange:handleImportXLSX}))
    ),
    CE('div',{className:'admin-section'},
      CE('h3',null,'🗑️ Réinitialiser la base de données'),
      CE('p',{style:{fontSize:12,color:'var(--label-color)',marginBottom:12}},'Vide uniquement le cache local. Le Google Sheet reste intact.'),
    ...
    return CE(FadeItem,{delay},CE('div',{style:{background:PBI_BG,borderRadius:6,padding:'14px',borderLeft:`4px solid ${color}`,boxShadow:'0 1px 6px rgba(0,0,0,.15)'}},
      CE('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}},
        CE('div',null,
          ...
          sub&&CE('div',{style:{fontSize:11,color:'var(--text-muted)',marginTop:4}},sub)
        ),
  ...
  return CE('div',{style:{fontFamily:"'Segoe UI',system-ui,sans-serif",minHeight:'60vh'}},
    ...
    CE('div',{style:{background:'#1e2132',borderRadius:'6px 6px 0 0',padding:'0 14px',display:'flex',alignItems:'center',gap:8,minHeight:44}},
      ...
      CE('div',{style:{flex:1,minWidth:0}},
        ...
        CE('span',{style:{fontSize:11,color:'var(--text-muted)'}}),'· Ateliers Inclusion Numérique'
      ),
      hasF&&CE('button',{
    ...
    CE('div',{style:{padding:14,background:isDark?'#0f1117':PBI_BG2,borderRadius:'0 0 6px 6px'}},
      ...
      page==='general'&&CE(React.Fragment,null,
        ...
        CE(CardPBI,{title:'Ateliers par mois',style:{marginBottom:12}},
          CE(PBIChart,{height:180,option:{backgroundColor:'transparent',
            ...
            legend:{data:['Réalisés','Planifiés','Annulés'],textStyle:{color:'var(--text-muted)',fontSize:9},bottom:0,icon:'roundRect',itemWidth:8,itemHeight:6},
            xAxis:{data:pMois.map(d=>d.mois),axisLine:{show:false},axisTick:{show:false},axisLabel:{color:'var(--text-muted)',fontSize:9}},
            yAxis:{splitLine:{lineStyle:{color:'#2d3250',type:'dashed'}},axisLabel:{color:'var(--text-muted)',fontSize:9},axisLine:{show:false},axisTick:{show:false}},
            series:[
        ...
        CE('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:12}},
          ...
          CE(CardPBI,{title:'Présents / mois',style:{}},
            CE(PBIChart,{height:150,option:{backgroundColor:'transparent',
              ...
              tooltip:{trigger:'axis',..._ecAPN,...ecTT},
              xAxis:{data:pMoisFilt.map(d=>d.mois),axisLine:{show:false},axisTick:{show:false},axisLabel:{color:'var(--text-muted)',fontSize:9}},
              yAxis:{splitLine:{lineStyle:{color:'#2d3250',type:'dashed'}},axisLabel:{color:'var(--text-muted)',fontSize:9},axisLine:{show:false},axisTick:{show:false}},
        ...
        pTheme.length>0&&CE(CardPBI,{title:'Top thématiques',style:{marginBottom:12}},
          CE(PBIChart,{height:Math.max(180,pTheme.length*24),option:{backgroundColor:'transparent',
            ...
            tooltip:{trigger:'axis',..._ecAPN,...ecTT},
            xAxis:{splitLine:{lineStyle:{color:'#2d3250',type:'dashed'}},axisLabel:{color:'var(--text-muted)',fontSize:9},axisLine:{show:false},axisTick:{show:false}},
            yAxis:{type:'category',data:pTheme.map(d=>d.name),axisLabel:{color:'var(--text-muted)',fontSize:9},axisLine:{show:false},axisTick:{show:false}},
        ...
        pOri.length>0&&CE(CardPBI,{title:'Organismes orienteurs'},
          CE(PBIChart,{height:160,option:{backgroundColor:'transparent',
            ...
            tooltip:{trigger:'axis',..._ecAPN,...ecTT},
            xAxis:{data:pOri.map(d=>d.name),axisLine:{show:false},axisTick:{show:false},axisLabel:{color:'var(--text-muted)',fontSize:8,rotate:-20,interval:0}},
            yAxis:{splitLine:{lineStyle:{color:'#2d3250',type:'dashed'}},axisLabel:{color:'var(--text-muted)',fontSize:9},axisLine:{show:false},axisTick:{show:false}},
            series:[{type:'bar',barMaxWidth:20,data:pOri.map(d=>d.count),
              ...
              label:{show:true,position:'top',color:'var(--text-muted)',fontSize:8}}]}}))
      ),
      ...
      page==='conseillers'&&CE(React.Fragment,null,
        ...
        CE(CardPBI,{title:'Réalisés par conseiller·ère — par mois',style:{marginBottom:12}},
          CE(PBIChart,{height:200,option:{backgroundColor:'transparent',
            ...
            tooltip:{trigger:'axis',..._ecAPN,...ecTT},
            legend:{data:CONS.map(c=>c.split(' ')[0]),textStyle:{color:'var(--text-muted)',fontSize:9},bottom:0,icon:'roundRect',itemWidth:8,itemHeight:6},
            xAxis:{data:pConsMois.map(d=>d.mois),axisLine:{show:false},axisTick:{show:false},axisLabel:{color:'var(--text-muted)',fontSize:9}},
            yAxis:{splitLine:{lineStyle:{color:'#2d3250',type:'dashed'}},axisLabel:{color:'var(--text-muted)',fontSize:9},axisLine:{show:false},axisTick:{show:false}},
      ...
      page==='territoire'&&CE(React.Fragment,null,
        pComm.length>0&&CE(CardPBI,{title:'Présents par commune',style:{marginBottom:12}},
          CE(PBIChart,{height:220,option:{backgroundColor:'transparent',
            ...
            tooltip:{trigger:'axis',..._ecAPN,...ecTT},
            xAxis:{splitLine:{lineStyle:{color:'#2d3250',type:'dashed'}},axisLabel:{color:'var(--text-muted)',fontSize:9},axisLine:{show:false},axisTick:{show:false}},
            yAxis:{type:'category',data:pComm.map(d=>d.name),axisLabel:{color:'var(--text-muted)',fontSize:9},axisLine:{show:false},axisTick:{show:false}},
        ...
        pComm.length>0&&CE(CardPBI,{title:'Ateliers par commune',style:{marginBottom:12}},
          CE(PBIChart,{height:180,option:{backgroundColor:'transparent',
            grid:{top:8,right:4,bottom:48,left:0,containLabel:true},
            tooltip:{trigger:'axis',..._ecAPN,...ecTT},
            xAxis:{data:pComm.map(d=>d.name),axisLine:{show:false},axisTick:{show:false},axisLabel:{color:'var(--text-muted)',fontSize:8,rotate:-20,interval:0}},
            yAxis:{splitLine:{lineStyle:{color:'#2d3250',type:'dashed'}},axisLabel:{color:'var(--text-muted)',fontSize:9},axisLine:{show:false},axisTick:{show:false}},
      ...
      CE('div',{style:{textAlign:'right',fontSize:10,color:'var(--text-muted)',marginTop:8}},
...
// ═══════════════════════════════════════════════════════
// VueThemePicker — Sélecteur de thème (modale + dots topbar)
// ═══════════════════════════════════════════════════════
function VueThemePicker({onClose}){
  const[sel,setSel]=React.useState(()=>localStorage.getItem('gdin_theme')||'teal');

  function pick(id){
    setSel(id);
    window.applyTheme(id);
    window.markThemeChosen();
  }

  function confirm(){
    window.applyTheme(sel);
    window.markThemeChosen();
    if(onClose)onClose();
    // Forcer re-render de l'app
    if(window._forceThemeUpdate)window._forceThemeUpdate();
  }

  const themes=window.GDIN_THEMES||[];

  return CE('div',{className:'theme-picker-overlay',onClick:e=>{if(e.target.className==='theme-picker-overlay')onClose&&onClose();}},
    CE('div',{className:'theme-picker-modal'},
      CE('div',{className:'theme-picker-header'},
        CE('h2',null,'🎨 Choisir un thème'),
        CE('button',{onClick:onClose,style:{background:'none',border:'none',cursor:'pointer',fontSize:20,color:'var(--text-muted)',fontFamily:'inherit'}},'✕')
      ),
      CE('div',{className:'theme-picker-grid'},
        themes.map(t=>{
          const active=sel===t.id;
          return CE('div',{
            key:t.id,
            className:'theme-picker-card'+(active?' active':''),
            style:{borderColor:active?t.neon:'var(--border)',boxShadow:active&&t.dark?`0 0 16px ${t.neon}55`:'none'},
            onClick:()=>pick(t.id)
          },
            // Preview mini
            CE('div',{className:'theme-picker-preview',style:{
              background:t.dark?t.sidebar:'linear-gradient(180deg,#1e3a8a,#172d72)',
              display:'flex',alignItems:'center',justifyContent:'center',
              padding:'8px'
            }},
              CE('div',{style:{display:'flex',gap:4,alignItems:'center'}},
                [t.neon,t.dark?'#22c55e':'#16a34a',t.dark?'#ef4444':'#dc2626'].map((c,i)=>CE('div',{key:i,style:{width:10,height:10,borderRadius:'50%',background:c,boxShadow:t.dark?`0 0 6px ${c}`:'none'}}))
              )
            ),
            CE('div',{className:'theme-picker-info'},
              CE('div',{className:'theme-picker-name',style:{color:t.neon,textShadow:t.dark?`0 0 10px ${t.neon}88`:'none'}},
                t.name,' ',
                CE('span',{style:{fontSize:9,fontWeight:700,textTransform:'uppercase',letterSpacing:'.08em',padding:'1px 5px',borderRadius:10,background:'rgba(255,255,255,.1)',color:'var(--text-muted)',marginLeft:4}},t.badge)
              ),
      ...
      CE('div',{style:{padding:'0 24px 20px',display:'flex',justifyContent:'flex-end',gap:10}},
        CE('button',{className:'btn btn-secondary',onClick:onClose},'Annuler'),
        CE('button',{
          className:'btn btn-primary',
          onClick:confirm,
          style:{boxShadow:`0 0 14px ${(themes.find(t=>t.id===sel)||themes[0]).neon}66`}
        },'✓ Appliquer ce thème')
      )
    )
  );
...
// ═══════════════════════════════════════════════════════
// VueThemeFirstLaunch — Écran de premier lancement
// ═══════════════════════════════════════════════════════
function VueThemeFirstLaunch({onDone}){
  const[sel,setSel]=React.useState('teal');
  const themes=window.GDIN_THEMES||[];
  ...
  return CE('div',{style:{
    ...
    CE('div',{style:{textAlign:'center',marginBottom:28}},
      CE('div',{style:{fontSize:48,marginBottom:12}},'🖥️'),
      CE('h1',{style:{fontFamily:'system-ui,sans-serif',fontSize:'clamp(20px,4vw,28px)',fontWeight:800,color:'#fff',letterSpacing:'-.02em',marginBottom:8}},'Bienvenue sur GDIN'),
      CE('p',{style:{fontSize:13,color:'#64748b',maxWidth:440,margin:'0 auto'}},'Choisissez le thème de votre interface. Vous pourrez le modifier à tout moment via le bouton 🎨 dans la barre du haut.')
    ),
    CE('div',{style:{
      display:'grid',
      gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))',
      gap:12,width:'100%',maxWidth:760,marginBottom:24
    }},
      themes.map(t=>{
        const active=sel===t.id;
        return CE('div',{
          ...
          style:{
            ...
            border:`2px solid ${active?t.neon:'rgba(255,255,255,.08)'}`,
            background:'#111827',
            transform:active?'translateY(-4px)':'none',
            transition:'all .2s',
            boxShadow:active&&t.dark?`0 8px 28px ${t.neon}33`:'none'
          }
        },
          CE('div',{style:{
            height:48,background:t.sidebar,
            display:'flex',alignItems:'center',justifyContent:'center',gap:6
          }},
            [t.neon,'#22c55e','#ef4444'].map((c,i)=>CE('div',{key:i,style:{width:10,height:10,borderRadius:'50%',background:c,boxShadow:t.dark?`0 0 6px ${c}`:'none'}}))
          ),
          CE('div',{style:{padding:'10px 12px'}},
            CE('div',{style:{fontSize:13,fontWeight:800,color:t.neon,marginBottom:2,textShadow:t.dark?`0 0 10px ${t.neon}66`:'none'}},t.name),
            CE('div',{style:{fontSize:11,color:'#64748b',lineHeight:1.4}},t.desc)
          )
        );
      })
    ),
    CE('button',{
      ...
      style:{
        ...
        fontSize:14,fontWeight:800,color:'#000',fontFamily:'inherit',
        background:(themes.find(t=>t.id===sel)||themes[0]).neon,
        boxShadow:`0 0 24px ${(themes.find(t=>t.id===sel)||themes[0]).neon}55`,
        transition:'all .2s'
      }
