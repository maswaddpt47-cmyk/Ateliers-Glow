...
// Design system : gdin-theme.css (tokens CSS)
// Inline styles → classes CSS, zéro couleur hardcodée
// ═══════════════════════════════════════════════════════════
...
(function injectCSS(){
  ...
  s.textContent=`
/* ── Saisie ─────────────────────────────────────────────── */
.g-sec{background:var(--bg-1);border-radius:14px;padding:18px 20px;margin-bottom:12px;
  box-shadow:0 2px 12px rgba(0,0,0,.18);border:1px solid var(--border)}
.g-lbl{font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;
  color:var(--text-3);display:block;margin-bottom:6px;font-family:'IBM Plex Mono',monospace}
.g-lbl.ac{color:var(--_ac,var(--cyan))}
.g-lbl.err{color:var(--red)}
.g-inp{width:100%;padding:11px 14px;border:2px solid var(--border);border-radius:10px;
  font-size:14px;color:var(--text);background:var(--bg-2);outline:none;
  box-sizing:border-box;transition:border-color .15s,background .15s;font-family:inherit}
.g-inp:focus{border-color:var(--cyan);background:var(--bg-1);box-shadow:0 0 0 3px var(--cyan-dim)}
.g-inp.err{border-color:var(--red)!important;background:var(--red-dim)!important}
.g-inp.num{font-family:'IBM Plex Mono',monospace;font-size:18px;font-weight:700;text-align:center}
.g-inp.sm{padding:8px 10px;font-size:12px}
.g-err{color:var(--red);font-size:11px;font-weight:600;display:block;margin-top:3px}
/* Statut pills */
.g-pill{padding:8px 16px;border-radius:20px;border:2px solid var(--border);
  ...
  font-family:inherit}
.g-pill.on{color:#fff;border-color:var(--_ac,var(--cyan));background:var(--_ac,var(--cyan))}
/* Matériel chips */
.g-chip{display:inline-flex;align-items:center;gap:5px;padding:7px 12px;
  border:2px solid var(--border);border-radius:20px;cursor:pointer;
  font-size:12px;font-weight:600;color:var(--text-2);background:var(--bg-2);
  transition:all .15s;user-select:none}
.g-chip.on{border-color:var(--_ac,var(--cyan));background:var(--bg-3);color:var(--_ac,var(--cyan))}
.g-chip input{display:none}
/* Mode toggle */
.g-modetog{display:flex;border:1.5px solid var(--border);border-radius:8px;
  overflow:hidden;margin-bottom:16px;width:fit-content}
.g-modetog button,.g-modetog-btn{padding:8px 20px;background:var(--bg-2);border:none;cursor:pointer;
  font-size:13px;font-weight:700;color:var(--text-3);transition:all .15s;font-family:inherit}
.g-modetog button.on,.g-modetog-btn.on{color:#fff}
/* Boutons saisie */
.g-btn-save{padding:13px 28px;border:none;border-radius:12px;font-size:14px;font-weight:700;
  color:#fff;background:var(--_ac,var(--cyan));cursor:pointer;
  display:inline-flex;align-items:center;gap:8px;transition:filter .15s;font-family:inherit}
.g-btn-save:hover:not(:disabled){filter:brightness(1.1)}
.g-btn-save:disabled{background:var(--bg-4);cursor:not-allowed;color:var(--text-3)}
.g-btn-cancel{padding:13px 24px;border:2px solid var(--border);border-radius:12px;
  cursor:pointer;font-size:14px;font-weight:600;color:var(--text-2);
  background:var(--bg-2);font-family:inherit;transition:all .15s}
.g-btn-cancel:hover{border-color:var(--border-2);color:var(--text)}
/* Badge conseiller */
.g-c-badge{display:inline-flex;align-items:center;gap:8px;padding:6px 14px;
  border-radius:20px;font-size:12px;font-weight:700;color:#fff;margin-bottom:14px}
/* Lot */
.g-lot-hdr{display:flex;align-items:center;gap:8px;font-size:13px;font-weight:700;
  border-radius:14px;padding:14px 18px;margin-bottom:12px;border:1px solid var(--border)}
.g-lot-row{display:grid;grid-template-columns:140px 100px 64px 1fr 32px;gap:8px;
  align-items:start;padding:9px 10px;border-radius:10px;border:1.5px solid var(--border);
  margin-bottom:6px;background:var(--bg-2)}
.g-lot-row.err{border-color:var(--red)!important;background:var(--red-dim)!important}
.g-lot-del{background:none;border:1px solid var(--border);border-radius:6px;color:var(--red);
  cursor:pointer;font-size:15px;height:32px;width:32px;display:flex;align-items:center;
  justify-content:center;transition:all .15s}
.g-lot-del:hover{background:var(--red-dim);border-color:var(--red)}
.g-lot-add{display:flex;align-items:center;justify-content:center;gap:6px;padding:10px;
  background:var(--bg-2);border-radius:10px;cursor:pointer;font-size:13px;
  font-weight:700;width:100%;margin-top:6px;transition:all .15s;font-family:inherit;
  border:2px dashed var(--border-2)}
.g-lot-add:hover{background:var(--bg-3);border-color:var(--cyan);color:var(--cyan)}
.g-lot-add.ac{color:var(--_ac,var(--cyan))}
/* ── KPI mini ───────────────────────────────────────────── */
.g-kpi{background:var(--bg-1);border-radius:8px;padding:10px 8px;text-align:center;
  border:1px solid var(--border);flex:1;min-width:60px}
.g-kpi .v{font-family:'IBM Plex Mono',monospace;font-size:22px;font-weight:700;line-height:1}
.g-kpi .l{font-size:10px;color:var(--text-3);margin-top:2px}
.g-kpi .p{font-size:10px;font-weight:600;margin-top:1px}
/* ── Panels ─────────────────────────────────────────────── */
.g-pan-close{background:none;border:none;font-size:18px;cursor:pointer;
  color:var(--text-3);padding:0;line-height:1;transition:color .15s}
.g-pan-close:hover{color:var(--text)}
.g-pan-title{font-size:13px;font-weight:700;color:var(--text);margin-bottom:8px}
.g-pan-sublbl{font-size:11px;font-weight:700;color:var(--text-3);margin-bottom:4px;
  text-transform:uppercase;letter-spacing:.06em;font-family:'IBM Plex Mono',monospace}
.g-pan-irrev{font-size:12px;color:var(--text-3);margin-bottom:16px}
/* ── Historique filtres ──────────────────────────────────── */
.g-flbl{font-size:11px;font-weight:700;color:var(--text-3);letter-spacing:.06em;
  margin-bottom:6px;font-family:'IBM Plex Mono',monospace;text-transform:uppercase;display:block}
.g-count{font-size:11px;color:var(--text-3);margin-top:8px}
.g-du-au{font-size:11px;font-weight:700;color:var(--text-3);white-space:nowrap}
/* ── Retard banner ──────────────────────────────────────── */
.g-retard{background:var(--orange-dim);border:1px solid rgba(249,115,22,.3);
  border-radius:10px;padding:10px 14px;margin-bottom:10px;display:flex;gap:10px;align-items:center}
.g-retard-t{font-weight:700;color:var(--orange);font-size:13px}
.g-retard-s{font-size:12px;color:var(--text-2)}
/* ── Period buttons ─────────────────────────────────────── */
.g-per{padding:4px 13px;border-radius:20px;font-size:12px;font-weight:600;cursor:pointer;
  border:1.5px solid var(--border);background:var(--bg-2);color:var(--text-3);
  font-family:inherit;transition:all .15s}
.g-per.on{background:var(--blue);color:#fff;border-color:var(--blue)}
.g-per.ac{background:var(--_ac,var(--cyan));color:#fff;border-color:var(--_ac,var(--cyan))}
.g-per:hover:not(.on):not(.ac){border-color:var(--border-2);color:var(--text)}
/* ── Roadmap ────────────────────────────────────────────── */
.g-rm-nav{font-size:18px;font-weight:800;color:var(--text);min-width:170px;
  text-align:center;font-family:'Syne',sans-serif}
.g-rm-flbl{font-size:12px;font-weight:600;color:var(--text-2);margin:0;white-space:nowrap}
/* ── Admin ──────────────────────────────────────────────── */
.g-ac-row{display:flex;align-items:center;gap:12px;padding:10px 14px;
  background:var(--bg-2);border-radius:10px;border:1.5px solid var(--border)}
.g-ac-name{flex:1;font-weight:700;font-size:14px;color:var(--text)}
.g-ac-hex{width:88px;font-family:'IBM Plex Mono',monospace;font-size:13px;font-weight:700;
  color:var(--text);padding:6px 10px;border:1.5px solid var(--border);border-radius:8px;
  background:var(--bg-1);text-align:center;outline:none}
.g-ac-reset{padding:4px 10px;border:1px solid var(--border);border-radius:6px;font-size:11px;
  color:var(--text-3);cursor:pointer;background:var(--bg-2);font-family:inherit;transition:all .15s}
.g-ac-reset:hover{color:var(--text);border-color:var(--border-2)}
/* ── Email rappels ──────────────────────────────────────── */
.g-email-row{display:flex;padding:8px 14px;margin-bottom:8px;background:var(--bg-2);
  border:1.5px solid var(--border);border-radius:8px;align-items:center;
  justify-content:space-between;gap:10px}
.g-email-lbl{font-size:12px;font-weight:700;color:var(--text-2)}
.g-email-inp{flex:1 1 140px;min-width:0;padding:5px 8px;border:1.5px solid var(--border);
  border-radius:6px;font-size:12px;color:var(--blue);background:var(--bg-1);
  font-family:'IBM Plex Mono',monospace;outline:none;transition:border-color .15s}
.g-email-inp:focus{border-color:var(--blue)}
.g-suppr{background:var(--red-dim);color:var(--red);border:none;border-radius:4px;
  padding:4px 10px;font-size:12px;cursor:pointer;font-family:inherit}
/* ── Agenda semaine ─────────────────────────────────────── */
.g-agenda-card{background:var(--bg-1);border-radius:8px;padding:6px 8px;margin-bottom:4px;
  cursor:pointer;box-shadow:0 1px 4px rgba(0,0,0,.15);font-size:11px;
  border-left-width:4px;border-left-style:solid}
.g-agenda-t{font-weight:600;color:var(--text);line-height:1.25}
.g-agenda-s{color:var(--text-3);font-size:10px}
.g-chip-status{font-size:11px;border-radius:20px;padding:3px 10px;font-weight:600}
/* ── Bouton dupliquer ───────────────────────────────────── */
.btn-dup{background:var(--blue-dim)!important;color:var(--blue)!important;
  border:1px solid rgba(59,130,246,.3)!important}
.btn-dup:hover{background:var(--blue)!important;color:#fff!important}
/* ── Divers ─────────────────────────────────────────────── */
.g-no-data{color:var(--text-3);font-size:12px;text-align:center;padding-top:20px;
  font-family:'IBM Plex Mono',monospace}
.g-irrev{font-size:12px;color:var(--text-3);margin-bottom:16px}
.g-date-inp{padding:5px 8px;border:1.5px solid var(--border);border-radius:6px;font-size:12px;
  background:var(--bg-2);color:var(--text);outline:none;transition:border-color .15s}
.g-date-inp:focus{border-color:var(--cyan)}
/* ── Bingo ──────────────────────────────────────────────── */
...
const CLOTURE_PRESETS=[
  {label:'✅ Réalisé',    statut:'Réalisé',     bg:'#16a34a',color:'var(--text)'},
  {label:'❌ Annulé',     statut:'Annulé',      bg:'#dc2626',color:'var(--text)'},
  {label:'🚫 Non réalisé',statut:'Non réalisé', bg:'#6b7280',color:'var(--text)'},
  {label:'📅 Reporté',    statut:'Reporté',     bg:'#d97706',color:'var(--text)'},
];
  ...
  return CE('div',{className:'listes-overlay',onClick:e=>{if(e.target.className==='listes-overlay')onClose();}},
    CE('div',{className:'listes-modal'},
      ...
      CE('div',{className:'listes-body'},
        activeTab==='conseillers'&&CE('div',{className:'g-email-row'},
          CE('span',{className:'g-email-lbl'},'📧 Rappels email'),
          CE('button',{
            className:'g-ac-reset',style:{},
        ...
        items.map((item,i)=>CE('div',{key:i,className:'listes-item',style:{flexWrap:'wrap'}},
          ...
          CE('div',{className:'listes-actions'},
            ...
            CE('button',{className:'g-suppr',onClick:()=>remove(i)},'Suppr.')
          )
        )),
...
function VueSaisie({entries,onSaved,onNewEntry,lists,editingId,onClearEdit,prefillData,onClearPrefill,accentColor}){
  ...
  // ── Labels ───────────────────────────────────────────────
  const Lbl=({t,err})=>CE('span',{className:'g-lbl'+(err?' err':' ac'),style:{'--_ac':ac}},t);
  const LblG=({t})=>CE('span',{className:'g-lbl'},t);

  // ── Style inputs ─────────────────────────────────────────
  const iCls=(err)=>'g-inp'+(err?' err':'');
  const sCls=(err)=>'g-inp'+(err?' err':'');
  const nCls=()=>'g-inp num';
  const taCls=(err)=>'g-inp'+(err?' err':'');
  const secSt={borderTop:`3px solid ${ac}`};

  // ── Statut pills ─────────────────────────────────────────
  const SDOTS={'Planifié':'#3b82f6','Réalisé':'#22c55e','Annulé':'#ef4444','Reporté':'#f59e0b','Non réalisé':'#94a3b8'};
  const statutPills=CE('div',{className:'g-sec',style:secSt},
    Lbl({t:'Statut *',err:!!errors.statut}),
    errors.statut&&CE('span',{className:'g-err'},errors.statut),
    CE('div',{style:{display:'flex',flexWrap:'wrap',gap:8}},
      statuts.map(s=>{
        const active=form.statut===s;
        return CE('button',{key:s,type:'button',
          className:'g-pill'+(active?' on':''),style:{'--_ac':ac,borderColor:active?ac:'',background:active?ac:''},onClick:()=>set('statut',s)},
  ...
  const champsCommuns=(frm,setFn,errs,entries_)=>CE('div',null,
    CE('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}},
      CE('div',null,
        ...
        errs.orienteur&&CE('span',{className:'g-err'},errs.orienteur)),
      CE('div',null,
        Lbl({t:'Commune *',err:!!errs.commune}),
        CE(ComboCommune,{value:frm.commune,onChange:v=>setFn('commune',v),hasError:!!errs.commune}),
        errs.commune&&CE('span',{className:'g-err'},errs.commune))
    ),
    CE('div',{style:{marginTop:12}},
      Lbl({t:'Lieu de l\'atelier *',err:!!errs.lieu}),
      CE('input',{type:'text',className:iCls(errs.lieu),value:frm.lieu,placeholder:'Salle, médiathèque, établissement…',onChange:e=>setFn('lieu',e.target.value)}),
      errs.lieu&&CE('span',{className:'g-err'},errs.lieu)
    ),
    CE('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12,marginTop:12}},
      CE('div',null,
        Lbl({t:'Conseiller *',err:!!errs.conseiller}),
        CE('select',{className:sCls(errs.conseiller),value:frm.conseiller,onChange:e=>setFn('conseiller',e.target.value)},
          CE('option',{value:'',disabled:true},'— Choisir —'),
          conseillers.map(c=>CE('option',{key:c,value:c},c))),
        errs.conseiller&&CE('span',{className:'g-err'},errs.conseiller)),
      CE('div',null,
        Lbl({t:'Type de public *',err:!!errs.public}),
        CE('select',{className:sCls(errs.public),value:frm.public,onChange:e=>setFn('public',e.target.value)},
          CE('option',{value:'',disabled:true},'— Choisir —'),
          publics.map(p=>CE('option',{key:p,value:p},p))),
        errs.public&&CE('span',{className:'g-err'},errs.public)),
      CE('div',null,
        LblG({t:'Résidence'}),
        CE('input',{type:'text',className:iCls(false),value:frm.residence,placeholder:'Commune d\'origine',onChange:e=>setFn('residence',e.target.value)}))
    ),
    CE('div',{style:{marginTop:12}},
      ...
      CE('div',{style:{display:'flex',flexWrap:'wrap',gap:8}},
        materiels.map(m=>{
          ...
          return CE('label',{key:m,className:'g-chip'+(chk?' on':''),style:{'--_ac':ac,borderColor:chk?ac:'',color:chk?ac:''}},CE('input',{type:'checkbox',checked:chk,onChange:()=>(modeLot?toggleLotMat:toggleMat)(m)}),m);
        })
      )
    ),
    CE('div',{style:{marginTop:12}},
      LblG({t:'Remarques'}),
      CE('input',{type:'text',className:iCls(false),value:frm.remarques,placeholder:'Notes libres',onChange:e=>setFn('remarques',e.target.value)}))
  );

  return CE('div',{style:{padding:'4px 0'}},
    // Badge conseiller coloré
    accentColor&&CE('div',{className:'g-c-badge',style:{background:ac}},
      editId?'✏️ Modifier':isDup?'📋 Duplication':modeLot?'🔄 Saisie par cycle':'⚡ Saisie One Shot'
    ),

    // Toggle One Shot / Cycle
    canToggle&&CE('div',{className:'g-modetog',style:{'--_ac':ac}},
      CE('button',{className:'g-modetog-btn'+((!modeLot)?' on':''),style:{background:!modeLot?ac:''},onClick:()=>{setModeLot(false);resetLot();}},'⚡ Saisie One Shot'),
      CE('button',{className:'g-modetog-btn'+(modeLot?' on':''),style:{background:modeLot?ac:''},onClick:()=>{setModeLot(true);reset();}},'🔄 Saisie par cycle')
    ),
    ...
    !modeLot&&CE('div',null,
      ...
      statutPills,
      // Date / Horaire / AM-PM
      CE('div',{className:'g-sec',style:secSt},
        CE('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr 80px',gap:12}},
          CE('div',null,
            Lbl({t:'Date *',err:!!errors.date}),
            CE('input',{type:'date',className:iCls(errors.date),value:form.date,onChange:e=>set('date',e.target.value)}),
            errors.date&&CE('span',{className:'g-err'},errors.date)),
          CE('div',null,
            Lbl({t:'Horaire *',err:!!errors.horaire}),
            CE('input',{type:'time',className:iCls(errors.horaire),value:form.horaire,onChange:e=>set('horaire',e.target.value)}),
            errors.horaire&&CE('span',{className:'g-err'},errors.horaire)),
          CE('div',null,
            Lbl({t:'AM/PM *',err:!!errors.ampm}),
            CE('select',{className:sCls(errors.ampm),value:form.ampm,onChange:e=>set('ampm',e.target.value)},
              CE('option',{value:'',disabled:true},'—'),CE('option',{value:'AM'},'AM'),CE('option',{value:'PM'},'PM')),
            errors.ampm&&CE('span',{className:'g-err'},errors.ampm))
        )
      ),
      // Champs communs
      CE('div',{className:'g-sec',style:secSt},champsCommuns(form,set,errors,entries)),
      // Thématique
      CE('div',{className:'g-sec',style:secSt},
        Lbl({t:'Thématique *',err:!!errors.thematique}),
        CE('textarea',{className:taCls(errors.thematique),style:{minHeight:84,resize:'vertical'},value:form.thematique,placeholder:"Thème abordé lors de l'atelier…",onChange:e=>set('thematique',e.target.value)}),
        errors.thematique&&CE('span',{className:'g-err'},errors.thematique)
      ),
      // Inscrits / Présents
      CE('div',{className:'g-sec',style:secSt},
        CE('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}},
          CE('div',null,
            ...
            CE('input',{type:'number',min:0,className:nCls()+(errors.inscrits?' err':''),value:form.inscrits,placeholder:'0',onChange:e=>set('inscrits',e.target.value)}),
            errors.inscrits&&CE('span',{className:'g-err'},errors.inscrits)),
          CE('div',null,
            LblG({t:'Présents'}),
            CE('input',{type:'number',min:0,className:nCls(),value:form.presents,placeholder:'0',onChange:e=>set('presents',e.target.value)}))
        )
      ),
      // Boutons
      CE('div',{style:{display:'flex',gap:12,marginTop:4}},
        CE('button',{className:'g-btn-save',style:{'--_ac':ac,background:saving?'var(--bg-4)':ac},onClick:handleSubmit,disabled:saving},
          saving?CE('span',null,CE('span',{className:'spinner'}),'Enregistrement…'):(editId?'💾 Modifier l\'atelier':'💾 Enregistrer l\'atelier')),
        CE('button',{className:'g-btn-cancel',onClick:reset},isDup||editId?'✖ Annuler':'✖ Réinitialiser')
      )
    ),
    ...
    // ══════════════════════════════
    modeLot&&CE('div',null,
      CE('div',{className:'g-lot-hdr',style:{background:acLight,borderTop:`3px solid ${ac}`,color:ac}},
        '🔄 ',lotRows.length,' atelier(s) à créer — tous "Planifié", inscrits/présents à compléter après'
      ),
      // Champs communs
      CE('div',{className:'g-sec',style:secSt},champsCommuns(lotForm,setLot,lotErrors,entries)),
      // Tableau des dates
      CE('div',{className:'g-sec',style:secSt},
        ...
        CE('div',{style:{display:'grid',gridTemplateColumns:'140px 100px 64px 1fr 32px',gap:8,padding:'0 4px',marginBottom:6}},
          CE('span',{className:'g-lbl'},'DATE *'),
          CE('span',{className:'g-lbl'},'HORAIRE *'),
          CE('span',{className:'g-lbl'},'AM/PM'),
          CE('span',{className:'g-lbl'},'THÉMATIQUE *'),
          CE('span',null)
        ),
        lotRows.map((row)=>{
          ...
          const inp=(type,val,key,err,ph)=>CE('input',{type,value:val,placeholder:ph||'',onChange:e=>setRow(row.id,key,e.target.value),className:'g-inp sm'+(err?' err':'')});
          return CE('div',{key:row.id,className:'g-lot-row'+(hasErr?' err':''),style:{borderColor:hasErr?'var(--red)':acLight,background:hasErr?'var(--red-dim)':acLight}},
            inp('date',row.date,'date',rErr.date),
            inp('time',row.horaire,'horaire',rErr.horaire),
            CE('select',{value:row.ampm,onChange:e=>setRow(row.id,'ampm',e.target.value),className:'g-inp sm'+(rErr.ampm?' err':''),style:{padding:'8px 4px'}},
              CE('option',{value:'',disabled:true},'—'),CE('option',{value:'AM'},'AM'),CE('option',{value:'PM'},'PM')),
            inp('text',row.thematique,'thematique',rErr.thematique,"Thème de la séance"),
            CE('button',{onClick:()=>removeRow(row.id),disabled:lotRows.length===1,className:'g-lot-del'},'×')
          );
        }),
        CE('button',{onClick:addRow,className:'g-lot-add',style:{color:ac,borderColor:acLight}},'＋ Ajouter une date')
      ),
      // Boutons
      CE('div',{style:{display:'flex',gap:12,marginTop:4}},
        CE('button',{className:'g-btn-save',style:{'--_ac':ac,background:saving?'var(--bg-4)':ac},onClick:handleSubmitLot,disabled:saving},
          saving?CE('span',null,CE('span',{className:'spinner'}),'Création…'):`💾 Créer ${lotRows.length} atelier(s)`),
        CE('button',{className:'g-btn-cancel',onClick:resetLot},'✖ Réinitialiser')
      )
    )
  );
}
...
function VueHistorique({entries,onEdit,onDelete,onRefresh,onDuplicate,initConseiller,onResetConseiller,canDelete,onChangeConseiller}){
  ...
  return CE('div',null,
    ...
    CE('div',{className:'kpi-row'},
      CE(FadeItem,{delay:0,style:{flex:'1 1 0',minWidth:0}},CE('div',{className:'g-kpi',style:{borderLeft:'3px solid var(--blue)',background:'var(--bg-1)'}},CE('div',{className:'v',style:{color:'var(--blue)'}},kpi.total),CE('div',{className:'l'},'Total'))),
      CE(FadeItem,{delay:0.08,style:{flex:'1 1 0',minWidth:0}},CE('div',{className:'g-kpi',style:{borderLeft:'3px solid var(--green)',background:'var(--bg-1)'}},CE('div',{className:'v',style:{color:'var(--green)'}},kpi.realises),CE('div',{className:'l'},'Réalisés'),CE('div',{className:'p',style:{color:'var(--green)'}},kpi.total?Math.round(kpi.realises/kpi.total*100)+'%':'-'))),
      CE(FadeItem,{delay:0.16,style:{flex:'1 1 0',minWidth:0}},CE('div',{className:'g-kpi',style:{borderLeft:'3px solid var(--red)',background:'var(--bg-1)'}},CE('div',{className:'v',style:{color:'var(--red)'}},kpi.annules),CE('div',{className:'l'},'Annulés'),CE('div',{className:'p',style:{color:'var(--red)'}},kpi.total?Math.round(kpi.annules/kpi.total*100)+'%':'-'))),
      CE(FadeItem,{delay:0.24,style:{flex:'1 1 0',minWidth:0}},CE('div',{className:'g-kpi',style:{borderLeft:'3px solid var(--blue)',background:'var(--bg-1)'}},CE('div',{className:'v',style:{color:'var(--blue)'}},kpi.inscrits),CE('div',{className:'l'},'Inscrits'))),
      CE(FadeItem,{delay:0.32,style:{flex:'1 1 0',minWidth:0}},CE('div',{className:'g-kpi',style:{borderLeft:'3px solid #d97706',background:'var(--bg-1)'}},CE('div',{className:'v',style:{color:'var(--orange)'}},kpi.presents),CE('div',{className:'l'},'Présents'),CE('div',{className:'p',style:{color:'var(--orange)'}},kpi.tx+'%')))
    ),
    // Alerte retards
    nRetard>0&&CE('div',{className:'g-retard'},
      CE('span',{style:{fontSize:20}},'⚠️'),
      CE('div',null,
        CE('div',{className:'g-retard-t'},`${nRetard} atelier(s) en attente de mise à jour`),
        CE('div',{className:'g-retard-s'},'Statut "Planifié" mais date déjà passée — cliquez sur un atelier pour le clôturer rapidement')
      )
    ),
    ...
    CE('div',{className:'card',style:{marginBottom:10}},
      CE('div',{className:'g-flbl'},'STATUT'),
      ...
      CE('div',{className:'g-count'},`${filtered.length} atelier(s) affiché(s) sur ${entries.length}`)
    ),
    // Liste des ateliers
    ...
    CE('div',{className:'side-panel'+(panel?' open':'')},
      panel&&CE(React.Fragment,null,
        CE('div',{className:'side-panel-header'},
          ...
          CE('button',{onClick:closePanel,className:'g-pan-close'},'✕')
        ),
        CE('div',{className:'side-panel-body'},
          ...
          CE('div',{className:'g-pan-title'},panel.thematique),
          ...
          panel.materiel&&panel.materiel.length>0&&CE('div',{style:{marginTop:10,marginBottom:4}},
            CE('div',{className:'g-pan-sublbl'},'MATÉRIEL'),
        ...
        CE('div',{className:'side-panel-footer'},
          ...
          onDuplicate&&CE('button',{className:'btn btn-secondary btn-sm',className:'btn btn-sm btn-dup',onClick:()=>{onDuplicate(panel);closePanel();}},'📋 Dupliquer'),
    ...
    confirmDel&&CE('div',{style:{position:'fixed',inset:0,background:'rgba(0,0,0,.5)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000}},
      CE('div',{className:'card',style:{width:360}},
        ...
        CE('p',{className:'g-pan-irrev'},'Cette action est irréversible.'),
    ...
    CE('div',{className:'card',style:{marginBottom:12}},
      CE('div',{style:{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10,marginBottom:12}},
        CE('div',{style:{display:'flex',alignItems:'center',gap:8}},
          ...
          CE('div',{className:'g-rm-nav'},`${MOIS_LONG[mo]} ${yr}`),
        ...
        CE('div',{style:{display:'flex',alignItems:'center',gap:8}},
          CE('label',{className:'g-rm-flbl'},'Conseiller :'),
      ...
      CE('div',{style:{display:'flex',gap:8,flexWrap:'wrap'}},
        CE('div',{className:'g-kpi',style:{flex:'1 1 70px',borderLeft:'3px solid var(--blue)',background:'var(--bg-1)'}},CE('div',{className:'v',style:{color:'var(--blue)',fontSize:20}},kpi.total),CE('div',{className:'l'},'Ateliers')),
        CE('div',{className:'g-kpi',style:{flex:'1 1 70px',borderLeft:'3px solid var(--green)',background:'var(--bg-1)'}},CE('div',{className:'v',style:{color:'var(--green)',fontSize:20}},kpi.realises),CE('div',{className:'l'},'Réalisés')),
        CE('div',{className:'g-kpi',style:{flex:'1 1 70px',borderLeft:'3px solid var(--blue)',background:'var(--bg-1)'}},CE('div',{className:'v',style:{color:'var(--blue)',fontSize:20}},kpi.planifies),CE('div',{className:'l'},'Planifiés')),
        CE('div',{className:'g-kpi',style:{flex:'1 1 70px',borderLeft:'3px solid var(--violet)',background:'var(--bg-1)'}},CE('div',{className:'v',style:{color:'var(--violet)',fontSize:20}},kpi.inscrits),CE('div',{className:'l'},'Inscrits')),
        CE('div',{className:'g-kpi',style:{flex:'1 1 70px',borderLeft:'3px solid var(--cyan)',background:'var(--bg-1)'}},CE('div',{className:'v',style:{color:'var(--cyan)',fontSize:20}},kpi.presents),CE('div',{className:'l'},'Présents'))
      )
    ),
    // ── Grille ──
      ...
      CE('div',{className:'cal-grid'},
        cells.map((day,idx)=>{
          ...
          return CE('div',{key:day,className:'cal-cell'+(isToday?' cal-today':'')},
            ...
            expanded&&hidden>0&&CE('div',{className:'cal-more',style:{color:'var(--red)'},onClick:ev=>{ev.stopPropagation();setExpandDay(null);}},`Réduire ▲`)
          );
        })
      )
    ),
...
// ═══════════════════════════════════════════════════════════
function NoData(){return CE('p',{className:'g-no-data'},'Aucune donnée');}
...
function _ec(){const d=isDark();return{
  ...
  LBL:{color:d?'#94a3b8':'#9ca3af',fontSize:10,fontFamily:"'IBM Plex Mono',monospace"},
  SPL:{lineStyle:{color:d?'rgba(255,255,255,0.06)':'rgba(0,0,0,0.06)',type:'dashed'}},
  LEG:{color:d?'#94a3b8':'#6b7280',fontSize:10},
  BDR:d?'#0d1220':'#ffffff',
};}
function EC_TT(){return _ec().TT;}
function EC_AXIS_BASE(){const t=_ec();return{axisLine:{show:false},axisTick:{show:false},axisLabel:t.LBL,splitLine:t.SPL};}
function EC_AXIS_LABEL(){return _ec().LBL;}
...
function BarChart({data,colors,height}){
  ...
  const option={
    ...
    series:[{type:'bar',barMaxWidth:44,
      ...
      label:{show:true,position:'top',color:_ec().LEG.color,fontSize:10,fontWeight:'bold'}
    }]
  };
...
function LineChart({data}){
  ...
  const option={
    ...
    series:[{type:'line',data:data.map(d=>d.value),smooth:true,symbol:'circle',symbolSize:5,
      lineStyle:{width:2.5,color:'var(--violet)'},
      itemStyle:{color:'var(--violet)'},
      areaStyle:{color:mkGrad('rgba(167,139,250,0.25)','rgba(167,139,250,0.02)')},
      label:{show:true,position:'top',color:_ec().LEG.color,fontSize:9,fontWeight:'bold'}
    }]
  };
...
function DualLineChart({data}){
  ...
  const option={
    ...
    tooltip:{trigger:'axis',...EC_TT(),...EC_APN},
    legend:{data:['Inscrits','Présents'],textStyle:{color:_ec().LEG.color,fontSize:11},bottom:0,icon:'circle',itemWidth:8,itemHeight:8},
    xAxis:{data:data.map(d=>d.label),...EC_AXIS_BASE(),splitLine:{show:false},axisLabel:{...EC_AXIS_LABEL(),rotate:data.length>8?-35:0,interval:0}},
    yAxis:{...EC_AXIS_BASE()},
    series:[
      {name:'Inscrits',type:'line',data:data.map(d=>d.inscrits),smooth:true,symbol:'circle',symbolSize:4,
       lineStyle:{width:2,color:'var(--blue)'},itemStyle:{color:'var(--blue)'},
      ...
      {name:'Présents',type:'line',data:data.map(d=>d.presents),smooth:true,symbol:'circle',symbolSize:4,
       lineStyle:{width:2,color:'var(--green)'},itemStyle:{color:'var(--green)'},
...
function RadialChart({data,colors,height=220}){
  ...
  const option={
    ...
    series:[{type:'bar',barMaxWidth:32,
      ...
      label:{show:true,position:'right',color:_ec().LEG.color,fontSize:10,fontWeight:'bold'}
    }]
  };
...
function DonutChart({data,colors,height=220}){
  ...
  const option={
    ...
    legend:{orient:'horizontal',bottom:0,textStyle:{color:_ec().LEG.color,fontSize:11},icon:'circle',itemWidth:8,itemHeight:8},
    series:[{type:'pie',radius:['38%','65%'],center:['50%','46%'],
      itemStyle:{borderRadius:5,borderColor:_ec().BDR,borderWidth:3},
      label:{show:true,position:'inside',color:'var(--text)',fontSize:10,fontWeight:'bold',formatter:p=>p.percent>6?Math.round(p.percent)+'%':''},
...
function StackedActivityChart({data}){
  ...
  const option={
    ...
    legend:{data:['Réalisés','Annulés','Planifiés'],textStyle:{color:_ec().LEG.color,fontSize:10},bottom:0,icon:'roundRect',itemWidth:10,itemHeight:8},
    ...
    series:[
      ...
      {name:'Planifiés',type:'bar',barMaxWidth:30,data:keys.map(k=>data[k].planifies),
        itemStyle:{color:'var(--blue)',borderRadius:[4,4,0,0]}}
    ]
  };
  return CE(EChart,{option,height:220});
}
...
function ConseillerBarChart({entries}){
  ...
  const option={
    ...
    legend:{data:['Réalisés','Planifiés','Annulés'],textStyle:{color:_ec().LEG.color,fontSize:10},bottom:0,icon:'roundRect',itemWidth:10,itemHeight:8},
    xAxis:{...EC_AXIS_BASE()},
    ...
    series:[
      {name:'Réalisés',type:'bar',barMaxWidth:14,data:data.map(d=>d.realises),
        ...
        label:{show:true,position:'right',color:_ec().LEG.color,fontSize:9,fontWeight:'bold'}},
  ...
  return CE('div',null,
    ...
    CE('div',{className:'card',style:{marginBottom:12,padding:'12px 16px'}},
      CE('div',{style:{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap'}},
        CE('span',{className:'g-flbl',style:{marginBottom:0,fontWeight:700}},'Période :'),
        ...
        CE('span',{style:{marginLeft:'auto',fontSize:11,color:'var(--text-3)'}},filtered.length+' ateliers · '+now.toLocaleDateString('fr-FR'))
      )
    ),

    // ── KPIs ──
    CE('div',{style:{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))',gap:12,marginBottom:16}},
      CE(KpiCard,{val:realises.length,lbl:'Ateliers réalisés',icon:'✅',color:'var(--green)',trend:mkTrend(realises.length,prevR.length),delay:0}),
      CE(KpiCard,{val:planifies.length,lbl:'Planifiés à venir',icon:'📅',color:'var(--blue)',delay:0.08}),
      CE(KpiCard,{val:presents,lbl:'Participants présents',icon:'👥',color:'var(--violet)',trend:mkTrend(presents,prevPresents),delay:0.16}),
      CE(KpiCard,{val:inscrits,lbl:'Inscrits total',icon:'📝',color:'var(--cyan)',delay:0.24}),
  ...
  return CE('div',null,
    ...
    CE('div',{className:'card',style:{marginBottom:12}},
      CE('div',{style:{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}},
        CE('span',{style:{fontSize:12,fontWeight:700,color:'var(--text-3)',whiteSpace:'nowrap'}},'📅 Période'),
        // Boutons années
    ...
    passes.length===0
      ...
      :CE('div',null,
          CE('div',{className:'kpi-grid'},
            CE(FadeItem,{delay:0,style:{display:'contents'}},CE('div',{className:'kpi',style:{borderLeft:'4px solid var(--green)',background:'var(--bg-1)',textAlign:'left'}},CE('div',{className:'val',style:{color:'var(--green)'}},total),CE('div',{className:'lbl'},'Ateliers réalisés'))),
            CE(FadeItem,{delay:0.08,style:{display:'contents'}},CE('div',{className:'kpi',style:{borderLeft:'4px solid var(--red)',background:'var(--bg-1)',textAlign:'left'}},CE('div',{className:'val',style:{color:'var(--red)'}},annules),CE('div',{className:'lbl'},'Annulés'))),
            CE(FadeItem,{delay:0.16,style:{display:'contents'}},CE('div',{className:'kpi',style:{borderLeft:'4px solid var(--violet)',background:'var(--bg-1)',textAlign:'left'}},CE('div',{className:'val',style:{color:'var(--violet)'}},totalPresents),CE('div',{className:'lbl'},'Participants présents'))),
            CE(FadeItem,{delay:0.24,style:{display:'contents'}},CE('div',{className:'kpi',style:{borderLeft:'4px solid var(--cyan)',background:'var(--bg-1)',textAlign:'left'}},CE('div',{className:'val',style:{color:'var(--cyan)'}},txPresence+'%'),CE('div',{className:'lbl'},'Taux de présence')))
          ),
...
function VueCarte({entries,active}){
  ...
  return CE('div',null,
    ...
    CE('div',{className:'card'},
      ...
      CE('div',{style:{display:'flex',gap:16,marginBottom:10,flexWrap:'wrap',fontSize:12,color:'var(--text-2)'}},
        CE('span',null,CE('span',{style:{display:'inline-block',width:12,height:12,borderRadius:'50%',background:'var(--green)',marginRight:5,verticalAlign:'middle'}}),'≥ 70% réalisés'),
        CE('span',null,CE('span',{style:{display:'inline-block',width:12,height:12,borderRadius:'50%',background:'var(--orange)',marginRight:5,verticalAlign:'middle'}}),'40 – 70%'),
        CE('span',null,CE('span',{style:{display:'inline-block',width:12,height:12,borderRadius:'50%',background:'var(--blue)',marginRight:5,verticalAlign:'middle'}}),'< 40%')
      ),
  ...
  return CE('div',null,
    CE('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}},
      CE('div',{style:{fontSize:13,color:'var(--text-3)'}},
        CE('span',{style:{marginRight:16}},CE('span',{style:{display:'inline-block',width:10,height:10,borderRadius:'50%',background:'var(--green)',border:'2px solid var(--green)',marginRight:4}}),'≥70% réalisés'),
        CE('span',{style:{marginRight:16}},CE('span',{style:{display:'inline-block',width:10,height:10,borderRadius:'50%',background:'var(--orange-dim)',border:'2px solid var(--orange)',marginRight:4}}),'40-70%'),
        CE('span',null,CE('span',{style:{display:'inline-block',width:10,height:10,borderRadius:'50%',background:'var(--blue-dim)',border:'2px solid var(--blue)',marginRight:4}}),'<40%')
      ),
    ...
    sel&&CE('div',{className:'card'},
      CE('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}},
        CE('h2',{style:{borderBottom:'none',marginBottom:0,paddingBottom:0}},CE('span',{style:{color:'var(--blue)'}},'📍 '+sel.nom),CE('span',{style:{fontSize:13,fontWeight:400,color:'var(--text-3)',marginLeft:8}},'— '+sel.total+' atelier(s)')),
        CE('button',{onClick:()=>setSelected(null),className:'g-pan-close',style:{}},'✕')
      ),
  ...
  return CE('div',{ref:adminRef},
    CE('div',{className:'card'},
      ...
      CE('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16,marginBottom:8}},
        CE(FadeItem,{delay:0,style:{display:'contents'}},CE('div',{className:'kpi',style:{borderLeft:'4px solid var(--blue)',background:'var(--bg-1)',textAlign:'left'}},CE('div',{className:'val',style:{color:'var(--blue)'}},entries.length),CE('div',{className:'lbl'},'Total ateliers'))),
        CE(FadeItem,{delay:0.1},CE('div',{className:'kpi',style:{borderLeft:'4px solid var(--green)',background:'var(--bg-1)',textAlign:'left'}},CE('div',{className:'val',style:{color:'var(--green)'}},entries.filter(e=>e.statut==='Réalisé').length),CE('div',{className:'lbl'},'Réalisés')))
      )
    ),
    // Couleurs conseillers
    CE('div',{className:'admin-section'},
      CE('h3',null,'🎨 Couleurs des conseillers'),
      CE('p',{style:{fontSize:12,color:'var(--text-2)',marginBottom:16}},'Personnalisez la couleur de chaque conseiller numérique. S\'applique au bandeau, au formulaire et aux cartes.'),
      CE('div',{style:{display:'flex',flexDirection:'column',gap:10}},
        (conseillersList||Object.keys(CONSEILLER_COLORS)).map(c=>{
          ...
          return CE('div',{key:c,className:'g-ac-row'},
            // Color picker visuel
            ...
            CE('button',{onClick:()=>{const orig=CONSEILLER_COLORS[c]||'#6B7280';setColorDraft(d=>({...d,[c]:orig}));},
              className:'g-ac-reset',style:{flexShrink:0}},'↩ Reset')
          );
        })
      ),
    ...
    visibility&&CE('div',{className:'admin-section'},
      ...
      CE('p',{style:{fontSize:12,color:'var(--text-2)',marginBottom:12}},'Choisissez les onglets visibles dans l\'interface conseiller.'),
    ...
    CE('div',{className:'admin-section'},
      ...
      CE('p',{style:{fontSize:12,color:'var(--text-2)',marginBottom:12}},'Vide uniquement le cache local. Le Google Sheet reste intact.'),
...
function VuePowerBI({entries, conseillers: conseillersList}){
    ...
    return CE(FadeItem,{delay},CE('div',{style:{background:PBI_BG,borderRadius:6,padding:'14px',borderLeft:`4px solid ${color}`,boxShadow:'0 1px 6px rgba(0,0,0,.15)'}},
      CE('div',{style:{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}},
        CE('div',null,
          ...
          sub&&CE('div',{style:{fontSize:11,color:'var(--text-3)',marginTop:4}},sub)
        ),
  ...
  const ecTT={backgroundColor:'#111827',borderColor:'#374151',textStyle:{color:'var(--text)',fontSize:11},extraCssText:'border-radius:8px;padding:10px 14px;box-shadow:none'};
  ...
  return CE('div',{style:{fontFamily:"'Segoe UI',system-ui,sans-serif",minHeight:'60vh'}},
    ...
    CE('div',{style:{background:'#1e2132',borderRadius:'6px 6px 0 0',padding:'0 14px',display:'flex',alignItems:'center',gap:8,minHeight:44}},
      ...
      CE('div',{style:{flex:1,minWidth:0}},
        CE('span',{style:{fontSize:12,fontWeight:700,color:'var(--text)'}},'Power BI '),
        CE('span',{style:{fontSize:11,color:'var(--text-3)'}}),'· Ateliers Inclusion Numérique'
      ),
      hasF&&CE('button',{
        onClick:()=>{setFCons([]);setFStat([]);setFMois([]);},
        style:{padding:'3px 10px',borderRadius:4,border:'none',background:'#ef4444',color:'var(--text)',fontSize:10,fontWeight:700,cursor:'pointer'}
      },'✕ Filtres'),
      CE('button',{
    ...
    showF&&CE('div',{style:{background:PBI_BG,border:`1px solid ${PBI_BORDER}`,borderTop:'none',padding:'12px 14px'}},
      CE('div',{style:{display:'flex',gap:16,flexWrap:'wrap'}},
        CE('div',null,
          CE('div',{style:{fontSize:10,fontWeight:700,color:'var(--text)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:6}}),'Mois',
        ...
        CE('div',null,
          CE('div',{style:{fontSize:10,fontWeight:700,color:'var(--text)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:6}}),'Conseiller·ère',
        ...
        CE('div',null,
          CE('div',{style:{fontSize:10,fontWeight:700,color:'var(--text)',textTransform:'uppercase',letterSpacing:'.06em',marginBottom:6}}),'Statut',
    ...
    CE('div',{style:{padding:14,background:isDark?'#0f1117':PBI_BG2,borderRadius:'0 0 6px 6px'}},
      ...
      page==='general'&&CE(React.Fragment,null,
        ...
        CE(CardPBI,{title:'Ateliers par mois',style:{marginBottom:12}},
          CE(PBIChart,{height:180,option:{backgroundColor:'transparent',
            ...
            legend:{data:['Réalisés','Planifiés','Annulés'],textStyle:{color:_ec().LEG.color,fontSize:9},bottom:0,icon:'roundRect',itemWidth:8,itemHeight:6},
            xAxis:{data:pMois.map(d=>d.mois),axisLine:{show:false},axisTick:{show:false},axisLabel:{color:'var(--text-3)',fontSize:9}},
            yAxis:{splitLine:{lineStyle:{color:'var(--text)',type:'dashed'}},axisLabel:{color:'var(--text-3)',fontSize:9},axisLine:{show:false},axisTick:{show:false}},
            series:[
        ...
        CE('div',{style:{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:12}},
          ...
          CE(CardPBI,{title:'Présents / mois',style:{}},
            CE(PBIChart,{height:150,option:{backgroundColor:'transparent',
              ...
              tooltip:{trigger:'axis',..._ecAPN,...ecTT},
              xAxis:{data:pMoisFilt.map(d=>d.mois),axisLine:{show:false},axisTick:{show:false},axisLabel:{color:'var(--text-3)',fontSize:9}},
              yAxis:{splitLine:{lineStyle:{color:'var(--text)',type:'dashed'}},axisLabel:{color:'var(--text-3)',fontSize:9},axisLine:{show:false},axisTick:{show:false}},
              series:[{type:'line',data:pMoisFilt.map(d=>d['Présents']),smooth:true,symbol:'none',
                lineStyle:{width:2,color:'var(--green)'},
        ...
        pTheme.length>0&&CE(CardPBI,{title:'Top thématiques',style:{marginBottom:12}},
          CE(PBIChart,{height:Math.max(180,pTheme.length*24),option:{backgroundColor:'transparent',
            ...
            tooltip:{trigger:'axis',..._ecAPN,...ecTT},
            xAxis:{splitLine:{lineStyle:{color:'var(--text)',type:'dashed'}},axisLabel:{color:'var(--text-3)',fontSize:9},axisLine:{show:false},axisTick:{show:false}},
            yAxis:{type:'category',data:pTheme.map(d=>d.name),axisLabel:{color:'var(--text-3)',fontSize:9},axisLine:{show:false},axisTick:{show:false}},
        ...
        pOri.length>0&&CE(CardPBI,{title:'Organismes orienteurs'},
          CE(PBIChart,{height:160,option:{backgroundColor:'transparent',
            ...
            tooltip:{trigger:'axis',..._ecAPN,...ecTT},
            xAxis:{data:pOri.map(d=>d.name),axisLine:{show:false},axisTick:{show:false},axisLabel:{color:'var(--text-3)',fontSize:8,rotate:-20,interval:0}},
            yAxis:{splitLine:{lineStyle:{color:'var(--text)',type:'dashed'}},axisLabel:{color:'var(--text-3)',fontSize:9},axisLine:{show:false},axisTick:{show:false}},
            series:[{type:'bar',barMaxWidth:20,data:pOri.map(d=>d.count),
              ...
              label:{show:true,position:'top',color:_ec().LEG.color,fontSize:8}}]}}))
      ),
      ...
      page==='conseillers'&&CE(React.Fragment,null,
        ...
        CE(CardPBI,{title:'Réalisés par conseiller·ère — par mois',style:{marginBottom:12}},
          CE(PBIChart,{height:200,option:{backgroundColor:'transparent',
            ...
            tooltip:{trigger:'axis',..._ecAPN,...ecTT},
            legend:{data:CONS.map(c=>c.split(' ')[0]),textStyle:{color:_ec().LEG.color,fontSize:9},bottom:0,icon:'roundRect',itemWidth:8,itemHeight:6},
            xAxis:{data:pConsMois.map(d=>d.mois),axisLine:{show:false},axisTick:{show:false},axisLabel:{color:'var(--text-3)',fontSize:9}},
            yAxis:{splitLine:{lineStyle:{color:'var(--text)',type:'dashed'}},axisLabel:{color:'var(--text-3)',fontSize:9},axisLine:{show:false},axisTick:{show:false}},
          ...
          CE('div',{style:{overflowX:'auto'}},
            CE('table',{style:{width:'100%',borderCollapse:'collapse',fontSize:11}},
              CE('thead',null,CE('tr',{style:{background:PBI_BG2}},
                ['','Total','Réal.','Ann.','Présents','Taux'].map(h=>CE('th',{key:h,style:{padding:'7px 8px',textAlign:'left',fontWeight:700,color:'var(--text)',borderBottom:'2px solid #e5e7eb',fontSize:10,whiteSpace:'nowrap'}},h))
              )),
              CE('tbody',null,CONS.map((c,i)=>{
                ...
                return CE('tr',{key:c,style:{background:i%2?PBI_BG2:PBI_BG}},
                  ...
                  CE('td',{style:{padding:'6px 8px',color:'var(--red)',fontWeight:600}},ann),
      ...
      page==='territoire'&&CE(React.Fragment,null,
        pComm.length>0&&CE(CardPBI,{title:'Présents par commune',style:{marginBottom:12}},
          CE(PBIChart,{height:220,option:{backgroundColor:'transparent',
            ...
            tooltip:{trigger:'axis',..._ecAPN,...ecTT},
            xAxis:{splitLine:{lineStyle:{color:'var(--text)',type:'dashed'}},axisLabel:{color:'var(--text-3)',fontSize:9},axisLine:{show:false},axisTick:{show:false}},
            yAxis:{type:'category',data:pComm.map(d=>d.name),axisLabel:{color:'var(--text-3)',fontSize:9},axisLine:{show:false},axisTick:{show:false}},
        ...
        pComm.length>0&&CE(CardPBI,{title:'Ateliers par commune',style:{marginBottom:12}},
          CE(PBIChart,{height:180,option:{backgroundColor:'transparent',
            grid:{top:8,right:4,bottom:48,left:0,containLabel:true},
            tooltip:{trigger:'axis',..._ecAPN,...ecTT},
            xAxis:{data:pComm.map(d=>d.name),axisLine:{show:false},axisTick:{show:false},axisLabel:{color:'var(--text-3)',fontSize:8,rotate:-20,interval:0}},
            yAxis:{splitLine:{lineStyle:{color:'var(--text)',type:'dashed'}},axisLabel:{color:'var(--text-3)',fontSize:9},axisLine:{show:false},axisTick:{show:false}},
      ...
      CE('div',{style:{textAlign:'right',fontSize:10,color:'var(--text-3)',marginTop:8}},
