...
function App(){
  ...
  const[theme,setTheme] = React.useState(()=>localStorage.getItem('gdin_theme')||'dark');

  // Sync theme depuis autres onglets
  React.useEffect(()=>{
    const h=e=>setTheme(e.detail.theme);
    window.addEventListener('gdin-theme-change',h);
    return()=>window.removeEventListener('gdin-theme-change',h);
  },[]);
    ...
    CE('div',{className:'app-content'},
      CE('div',{className:'app-topbar'},
        ...
        CE('div',{className:'app-topbar-right'},
          ...
          loading&&CE('span',{style:{fontSize:11,color:'var(--text-3)',display:'flex',alignItems:'center',gap:4}},
            CE('span',{className:'spinner',style:{borderTopColor:'var(--cyan)',borderColor:'var(--border)'}}), 'Chargement…'),
