...
function App(){
  ...
  const[firstLaunch,setFirstLaunch] = React.useState(()=>window.isFirstThemeLaunch&&window.isFirstThemeLaunch());
  // Force re-render quand thème change
  const[themeKey,setThemeKey] = React.useState(0);
  React.useEffect(()=>{ window._forceThemeUpdate=()=>setThemeKey(k=>k+1); return()=>{window._forceThemeUpdate=null;}; },[]);
  ...
  // ── Premier lancement — choix du thème ──────────────────────────
  if(firstLaunch){
    return CE(VueThemeFirstLaunch,{onDone:()=>{setFirstLaunch(false);}});
  }
              ...
              !loading&&entries.length>0&&CE('div',{className:'accueil-stats'},
                lists.conseillers.map(c=>{
                  ...
                  return CE('div',{key:c,className:'accueil-stat-chip',style:{background:color+'12',border:`1px solid ${color}30`,color}},
                    ...
                    CE('span',{style:{fontSize:10,color:'var(--text-muted)'}},'ce mois'),
  ...
  return CE('div',{className:'app-shell'},
    ...
    CE('div',{className:'app-content'},
      CE('div',{className:'app-topbar'},
        ...
        CE('div',{className:'app-topbar-right'},
          ...
          loading&&CE('span',{style:{fontSize:11,color:'var(--text-muted)',display:'flex',alignItems:'center',gap:4}},
          ...
          filtreConseiller&&CE('div',{style:{position:'relative'},onMouseDown:e=>e.stopPropagation()},
            ...
            showPicker&&CE('div',{className:'conseiller-picker'},
              lists.conseillers.map(c=>CE('div',{
                ...
                c,
                c===filtreConseiller&&CE('span',{style:{marginLeft:'auto',fontSize:11,color:'var(--text-muted)'}},'✓')
              ))
            )
          )
        )
    ...
    CE('div',{id:'toast',className:'toast',style:{opacity:0}}),
    showThemePicker&&CE(VueThemePicker,{onClose:()=>setShowThemePicker(false)})
  );
}
  ...
  return CE('div',{onMouseMove:handleMouseMove},
    ...
    CE('div',{className:'card',style:{marginBottom:12}},
      ...
      CE('div',{style:{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10,marginBottom:14}},
        ...
        CE('div',{style:{display:'flex',background:'var(--bg-card2)',borderRadius:8,padding:3,gap:2}},
      ...
      CE('div',{style:{
        ...
        CE('span',{style:{fontSize:11,fontWeight:700,color:'var(--label-color)',whiteSpace:'nowrap'}},'📅 Période'),
        // Raccourcis
        ...
        CE('div',{style:{display:'flex',gap:6,alignItems:'center',flexWrap:'wrap'}},
          CE('input',{type:'date',value:dateFrom,
            ...
            style:{
              ...
              borderRadius:6,fontSize:12,color:'var(--text)',background:'var(--card-bg)',cursor:'pointer',outline:'none'
            }
          }),
          CE('span',{style:{fontSize:12,color:'var(--text-muted)',fontWeight:600}},'→'),
      ...
      CE('div',{style:{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10,marginBottom:14}},
        [
          {label:'Ateliers',value:kpis.total,color:'var(--neon)'},
          ...
          CE('div',{style:{fontSize:11,color:'var(--text-muted)',marginTop:2}},k.label)
        ))
      ),
      ...
      CE('div',{style:{display:'flex',gap:12,flexWrap:'wrap',alignItems:'center'}},
        CE('div',{style:{display:'flex',gap:5,alignItems:'center',flexWrap:'wrap'}},
          CE('span',{style:{fontSize:11,color:'var(--text-muted)',fontWeight:600}},'Conseiller :'),
        ...
        CE('div',{style:{display:'flex',gap:5,alignItems:'center',flexWrap:'wrap'}},
          CE('span',{style:{fontSize:11,color:'var(--text-muted)',fontWeight:600}},'Statut :'),
    ...
    viewMode==='gantt'&&CE('div',{className:'card',style:{padding:0,overflow:'hidden'}},
      // En-tête mois
      CE('div',{style:{display:'flex',borderBottom:'1px solid var(--border)',background:'var(--bg-card2)'}},
        CE('div',{style:{width:170,minWidth:170,padding:'8px 16px',fontSize:10,fontWeight:700,color:'var(--text-muted)',textTransform:'uppercase',letterSpacing:'.08em',borderRight:'1px solid #e2e8f0'}},'Conseiller'),
        CE('div',{style:{flex:1,position:'relative',height:32}},
          ...
          }},CE('span',{style:{fontSize:10,fontWeight:700,color:'var(--text-muted)',whiteSpace:'nowrap'}},m.label))),
          todayPos!==null&&CE('div',{style:{
            ...
            fontWeight:700,whiteSpace:'nowrap',background:'var(--card-bg)',padding:'0 2px',borderRadius:2
          }},'Auj.'))
        )
      ),
      // Lignes conseillers
      displayedConseillers.map((conseiller,ci)=>{
        ...
        return CE('div',{key:conseiller,style:{
          ...
          CE('div',{style:{
            ...
            CE('div',null,
              CE('div',{style:{fontSize:12,fontWeight:700,color:'var(--text)',lineHeight:1.2}},conseiller.split(' ')[0]),
              CE('div',{style:{fontSize:10,color:'var(--text-muted)'}},conseiller.split(' ').slice(1).join(' '))
            ),
            CE('span',{style:{
      ...
      // Légende
      CE('div',{style:{display:'flex',gap:14,padding:'10px 16px',background:'var(--bg-card2)',borderTop:'1px solid #e2e8f0',flexWrap:'wrap'}},
        Object.entries(STATUT_STYLE).map(([s,st])=>CE('div',{key:s,style:{display:'flex',alignItems:'center',gap:5,fontSize:11,color:'var(--label-color)'}},
        ...
        filtered.length===0&&CE('span',{style:{fontSize:12,color:'var(--text-muted)',marginLeft:'auto'}},'Aucun atelier pour cette période')
      )
    ),
    ...
    viewMode==='densite'&&(()=>{
      ...
      return CE('div',{className:'card'},
        CE('div',{style:{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16,flexWrap:'wrap',gap:8}},
          CE('h3',{style:{margin:0,fontSize:14,color:'var(--text)'}},'Ateliers par mois & conseiller'),
          CE('div',{style:{display:'flex',gap:12,fontSize:11,color:'var(--text-muted)'}},
            CE('span',null,'▪ Clair = planifiés/autres'),
            CE('span',null,'▪ Plein = réalisés')
          )
        ),
        visibleMonths.length===0
          ?CE('p',{style:{color:'var(--text-muted)',fontSize:13}},'Aucune période sélectionnée.')
          :CE('div',{style:{overflowX:'auto'}},
            CE('table',{style:{width:'100%',borderCollapse:'collapse',minWidth:Math.max(400,visibleMonths.length*52+140)}},
              CE('thead',null,CE('tr',null,
                CE('th',{style:{width:140,minWidth:140,textAlign:'left',padding:'6px 10px',fontSize:11,fontWeight:700,color:'var(--text-muted)',borderBottom:'2px solid var(--border)'}},'Conseiller'),
                visibleMonths.map(({label},i)=>CE('th',{key:i,style:{textAlign:'center',padding:'6px 8px',fontSize:11,fontWeight:700,color:'var(--text-muted)',borderBottom:'2px solid var(--border)',whiteSpace:'nowrap'}},label))
              )),
              CE('tbody',null,
                  ...
                  return CE('tr',{key:conseiller},
                    CE('td',{style:{padding:'8px 10px',borderBottom:'1px solid #f0f4f8',whiteSpace:'nowrap'}},
                      CE('div',{style:{display:'flex',alignItems:'center',gap:6}},
                        ...
                        CE('span',{style:{fontSize:12,fontWeight:700,color:'var(--text)'}},conseiller.split(' ')[0])
                      )
                    ),
                    byMonth.map((m,mi)=>{
  ...
  return CE('div',null,
    ...
    CE('div',{className:'card',style:{marginBottom:12,padding:'12px 16px'}},
      ...
      CE('div',{style:{display:'flex',alignItems:'center',gap:6,flexWrap:'wrap',marginTop:8,paddingTop:8,borderTop:'1px solid #f1f5f9'}},
        CE('span',{style:{fontSize:12,fontWeight:700,color:'var(--text-muted)'}},'Mois :'),
        ...
        selectedMonths.length>0&&CE('button',{onClick:()=>setSelectedMonths([]),style:{
          ...
          border:'1.5px solid #fca5a5',background:'var(--card-bg)',color:'#ef4444',fontWeight:600
        }},'✕ Effacer')
      )
    ),
...
function VueDashboardTabs({entries, conseillers}){
  ...
  return CE('div',null,
    CE('div',{className:'dashboard-tab-bar'},
